...
Restart_zhenxun_bot() {
   Stop_zhenxun_bot
   Start_zhenxun_bot_portry
   echo -e "${lnfo} zhenxun_bot 开始运行"
}
...
Install_sshd() {
    echo -e "${Info} 开始安装ssh服务"
        ...
        mv /etc/ssh/sshd_config /etc/ssh/sshd_config_bak && wget https://gitee.com/soloxiaoye/zhenxun_bot_tool/attach_files/1095259/download/sshd_config -qO /etc/ssh/sshd_config
        echo -e "${Info} 请设置ssh服务端口"
    read -erp "Port:" Portt
    [[ -z "$Port" ]] && Port=""
    cd /etc/ssh && sed -i "s|Port.*|Port:$Port/g" sshd_config || echo -e "${Error} 配置文件不存在！请检查ssh是否安装正确!"
    echo -e "${info} 设置成功!端口: ${Port}"
...
EOF
    ...
    echo -e "${lnfo} 成功创建用户名zhenxun,密码zxpassword"
    echo -e "${lnfo} 成功创建数据库:zhenxun"
}
...
menu_termux() {
 ...
 ${Green_font_prefix} 1.${Font_color_suffix} 安装 zhenxun_bot + go-cqhttp
————————————
 ${Green_font_prefix} 2.${Font_color_suffix} 启动 postgresql 数据库
 ${Green_font_prefix} 3.${Font_color_suffix} 停止 postgresql 数据库
 ${Green_font_prefix} 4.${Font_color_suffix} 重启 postgresql 数据库
 ${Green_font_prefix} 5.${Font_color_suffix} 创建 数据库 (待添加功能)
————————————
 ${Green_font_prefix} 6.${Font_color_suffix} 安装 sshd 服务
 ${Green_font_prefix} 7.${Font_color_suffix} 修改 apt源为 北外源
 ${Green_font_prefix} 8.${Font_color_suffix} 修改 apt源为 清华源
————————————
